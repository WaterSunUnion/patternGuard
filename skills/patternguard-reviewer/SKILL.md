...
Open `.patternguard/patternguard-handoff.yaml` and read `retry_count`.
...
### Solo
**When:** A single file was changed and it has no declared siblings in `.patternguard/CODEBASE_INDEX.yaml`.
...
3. **Index conformance** — If the file's pattern is declared in `.patternguard/CODEBASE_INDEX.yaml`, does the file conform to the pattern description?
...
6. **Pattern header integrity** — If the file is listed in `.patternguard/CODEBASE_INDEX.yaml`, or the review envelope declares a `pattern`, the file must contain:
   - `// Patterns: <pattern-name>`
   - `// Index: .patternguard/CODEBASE_INDEX.yaml`
   If either line is missing or names the wrong pattern/index path, emit `changes-needed`. If the file is generated, also require the full generated header:
   - `// Code generated by <generator>. DO NOT EDIT.`
   - `// Patterns: <pattern-name>`
   - `// Index: .patternguard/CODEBASE_INDEX.yaml`
   - A line directing agents to edit the template and regenerate instead of editing the generated file directly.
...
If the root cause is sibling inconsistency (check #2) and the pattern has 3 or more siblings with no template-backed generator path (`template:` plus `generator.command`) in `.patternguard/CODEBASE_INDEX.yaml`, append this to your findings:
```
Extraction candidate: pattern `<name>` has N siblings with no template-backed generator path. Sibling inconsistency here is a signal that this logic should be marked as a template candidate, or promoted to template mode if a generator command already exists. If no generator exists, keep duplicated execution and surface generator setup as the next step.
```

### `ESCALATE`
Emit this when:
- `retry_count` in `.patternguard/patternguard-handoff.yaml` is 3 or higher (checked in Step 1)
